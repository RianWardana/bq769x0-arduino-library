...
// CRC calculation taken from LibreSolar mbed firmware (thanks to mikethezipper for finding it out)
...
void bq769x0::updateVoltages()
{
  ...
  for (int i = 0; i < numberOfCells; i++) {
    if (crcEnabled == true) {
      ...
      if (crc != buf[3]) {
        Serial.println("CRC check failed - VCxLO data and its CRC doesn't match");
...
void bq769x0::printRegisters()
{
  ...
  Serial.print(F("0x03 CELLBAL3:  "));
  Serial.println(byte2char(readRegister(CELLBAL3)));
